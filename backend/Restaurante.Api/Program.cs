...
var key = Encoding.UTF8.GetBytes(rawJwtKey);
...
using (var scope = app.Services.CreateScope())
{
    ...
    await db.Database.ExecuteSqlRawAsync("""
        ...
        CREATE INDEX IF NOT EXISTS idx_cash_movements_session_date ON cash_movements(restaurant_id, cash_session_id, created_at);

        CREATE TABLE IF NOT EXISTS customers (
            id UUID PRIMARY KEY, restaurant_id UUID NOT NULL REFERENCES restaurants(id), name VARCHAR(160) NOT NULL,
            phone VARCHAR(30), email VARCHAR(180), birth_date DATE, points INTEGER NOT NULL DEFAULT 0,
            cashback_balance NUMERIC(12,2) NOT NULL DEFAULT 0, active BOOLEAN NOT NULL DEFAULT TRUE,
        ...
        CREATE TABLE IF NOT EXISTS loyalty_movements (
            ...
            order_id UUID NULL REFERENCES orders(id) ON DELETE SET NULL, type VARCHAR(20) NOT NULL,
            points INTEGER NOT NULL DEFAULT 0, cashback NUMERIC(12,2) NOT NULL DEFAULT 0,
            description VARCHAR(240) NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
        );
        CREATE INDEX IF NOT EXISTS idx_loyalty_customer_date ON loyalty_movements(restaurant_id, customer_id, created_at);

        CREATE TABLE IF NOT EXISTS coupons (
            id UUID PRIMARY KEY, restaurant_id UUID NOT NULL REFERENCES restaurants(id), code VARCHAR(40) NOT NULL,
            description VARCHAR(180) NOT NULL, discount_type VARCHAR(20) NOT NULL, value NUMERIC(12,2) NOT NULL,
            minimum_order_value NUMERIC(12,2) NOT NULL DEFAULT 0, max_uses INTEGER, uses INTEGER NOT NULL DEFAULT 0,
    ...
    if (app.Environment.IsDevelopment())
    {
        ...
        if (demoAdmin is null)
        {
            ...
            {
                RestaurantId = demoRestaurant.Id, Name = "Administrador", Email = "admin@demo.local",
                Role = "ADMIN", Active = true, PasswordHash = BCrypt.Net.BCrypt.HashPassword("Admin@123")
            });
            await db.SaveChangesAsync();
        }

        var existingIngredientNames = await db.Ingredients.Where(x => x.RestaurantId == demoRestaurant.Id)
        ...
        var missingIngredients = demoIngredients.Where(x => !existingIngredientNames.Contains(x.Name, StringComparer.OrdinalIgnoreCase)).ToList();
        ...
        var ingredientsByName = await db.Ingredients.Where(x => x.RestaurantId == demoRestaurant.Id)
            .ToDictionaryAsync(x => x.Name, StringComparer.OrdinalIgnoreCase);
        var hamburger = await db.Products.SingleOrDefaultAsync(x => x.RestaurantId == demoRestaurant.Id && x.Name == "Hambúrguer Artesanal");
        ...
        var soda = await db.Products.SingleOrDefaultAsync(x => x.RestaurantId == demoRestaurant.Id && x.Name == "Refrigerante");
